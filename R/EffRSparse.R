...
# Input:
#' @param A
#' Weighted adjacency matrix of the class dgCMatrix
# Output:
#' @return Graph laplacian, \code{L}, of the class dgCMatrix.
#'
#' @export
Lap <- function(A){ # input is a sparse matrix
...
#'
#' @export
# Find signed-edge vertex incidence matrix, Bmxn
...
# Output:
#' @return Return either exact or approximate effective resistances for each edge in the same order as "weight" in edge list.
#'
#' @export
...
#'
#' @export
EffRSparse <- function(n, E_List, q, effR){
  m = dim(E_List)[1]
  P = matrix(0L, 1, m)
